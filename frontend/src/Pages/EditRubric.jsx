...
const EditRub = () => {
  ...
  const { updateAssessment, currentAssessment } = assessmentFrameworkStore();
  ...
  useEffect(() => {
    if (currentAssessment) {
      const transformedData = {
        ...
        scoringScale: currentAssessment.scoringScale.map(
          ({ description, score }) => ({
            description,
            score,
          })
        ),
        criteriaArray: currentAssessment.criteriaArray.map(
  ...
  const onSubmit = async (data) => {
    try {
      await updateAssessment(currentAssessment._id, data);
    ...
    <div className="ml-10 lg:ml-64 h-screen overflow-auto mt-0">
      <div className="mt-20">
        <div className="grid place-items-center">
          <h1 className="text-2xl font-bold mb-8">Edit Rubric</h1>
        </div>
